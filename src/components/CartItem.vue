...
<!--      图片-->
      <div style="display: inline-block;margin-right: 20px">
        <router-link :to="'bookview/'+cart.bookId">
...
export default {
  ...
  methods:{
    ...
    //跳转到支付页面
    pay(){
      let book = {id: this.cart.bookId,name: this.cart.bookName,imgs: this.cart.img,discount: this.cart.discount}
      this.$router.push({name: 'preOrder',query: {book: JSON.stringify(book), realPrice: this.realPrice, num: this.cart.count, standard: this.cart.standard, cartId: this.cart.id}})
    },
  }
}
</script>
