...
export default {
    ...
    created() {
        ...
        this.request.get("/api/book/" + this.bookId).then((res) => {
            if (res.code === "200") {
                this.book = res.data;
                let discount = this.book.discount;
                if (discount < 1) {
        ...
        this.request.get("/api/book/standard/" + this.bookId).then((res) => {
            if (res.code === "200") {
                ...
                //默认选择第一个标准
                console.log("规格是： ", standards)
                this.price = this.getPriceRange(standards);
            } else {
                //没有规格
                this.price = this.book.price;
                this.store = this.book.store;
                this.showStore = true;
            }
        });
    },
        ...
        realPrice: function () {
            if (this.book.discount < 1) {
                ...
                if (isNaN(this.price)) {
                    ...
                    return (this.price * this.book.discount).toFixed(2);
                }
            }
            return this.price;
        },
